...
export default function ChefMedicalActivitesPage() {
  return (
...
function Contenu() {
  ...
  async function chargerActivites() {
    ...
    try {
      const data = await listerMedecins({ par_page: 100 });
  ...
  return (
    <div className="min-h-screen space-y-6 apparition pb-20">
      <div>
        <p className="text-lagune font-semibold text-sm uppercase tracking-wider">📊 Activités</p>
        <h1>Activités de l'équipe médicale</h1>
        <p className="text-ardoise-clair mt-2">Suivez l'activité et les performances de vos agents de santé</p>
      </div>
      {erreur && <Alerte variante="erreur">{erreur}</Alerte>}
      {chargement ? (
        <div className="text-center py-12"><div className="animate-spin w-8 h-8 border-4 border-lagune border-t-transparent rounded-full mx-auto mb-3"></div><p className="text-ardoise-clair">Chargement...</p></div>
      ) : (
        <div className="space-y-4">
              ...
              {agents.map((agent) => (
                <div key={agent.id} className="flex items-center justify-between p-4 bg-sable rounded-lg">
                  <div className="flex items-center gap-4">
                    <div className="w-12 h-12 rounded-full bg-lagune/10 flex items-center justify-center text-lagune font-bold">{(agent.prenom[0] || "") + (agent.nom[0] || "")}</div>
                    <div>
                  ...
                  <div className="text-right">
                    <p className="text-sm font-semibold text-lagune">Dernière activité</p>
          ...
          <div className="grid md:grid-cols-3 gap-4">
            <Carte className="text-center p-6"><p className="text-3xl font-bold text-lagune">{agents.filter(a => a.est_actif).length}</p><p className="text-xs text-ardoise-clair mt-2">Agents actifs aujourd'hui</p></Carte>
      ...
      <Link href="/chef-medical"><Bouton variante="ghost">← Retour au tableau de bord</Bouton></Link>
    </div>
  );
}