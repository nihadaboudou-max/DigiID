...
function Contenu() {
  ...
  // Helper pour la couleur du badge selon l'action
  const getBadgeVariante = (type: string) => {
    if (type.includes("verification") || type.includes("recherche") || type.includes("consultation")) return "lagune";
    if (type.includes("signalement") || type.includes("alerte") || type.includes("erreur")) return "terre";
    if (type.includes("connexion")) return "succes";
    return "ocre";
  };
  ...
  return (
    <div className="space-y-6 apparition pb-20">
      <div>
        <p className="text-lagune font-semibold text-sm uppercase tracking-wider">🛡️ Supervision Médicale</p>
        <h1>Journal d'Audit de l'Équipe</h1>
        <p className="text-ardoise-clair mt-2">
          Historique complet de toutes les actions effectuées par vos agents médicaux.
        </p>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div>
            ...
            >
              ...
              <option value="modification_profil">Modification de profil</option>
              <option value="consultation_dossier">Consultation de dossier</option>
              <option value="creation_ordonnance">Création d'ordonnance</option>
            </select>
          </div>
          <div>
      ...
      {/* ✅ CORRECTION : Lien de retour vers le dashboard médical */}
