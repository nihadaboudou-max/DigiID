"use client";

import { useState } from "react";
import Link from "next/link";
import { useRouter } from "next/navigation";
import { EnvelopperEspaceProtege } from "@/composants/layouts/EnvelopperEspaceProtege";
import { Carte } from "@/composants/commun/Carte";
import { Bouton } from "@/composants/commun/Bouton";
import { ChampSaisie } from "@/composants/commun/ChampSaisie";
import { creerDossier, verifierPatient } from "@/services/medical";
import type { VerificationPatient } from "@/services/medical";

export default function NouveauDossierPage() {
  return (
    <EnvelopperEspaceProtege rolesAutorises={["agent_medical", "chef_medical"]}>
      <Contenu />
    </EnvelopperEspaceProtege>
  );
}

function Contenu() {
  const router = useRouter();
  
  const [patient_nom, setPatientNom] = useState("");
  const [patient_prenom, setPatientPrenom] = useState("");
  const [patient_digiid, setPatientDigiid] = useState("");
  const [motif, setMotif] = useState("");
  const [diagnostic, setDiagnostic] = useState("");
  const [hopital, setHopital] = useState("");
  
  const [etape, setEtape] = useState<"recherche" | "formulaire" | "confirmation">("recherche");
  const [envoi, setEnvoi] = useState(false);
  const [erreur, setErreur] = useState("");
  const [patientVerifie, setPatientVerifie] = useState<VerificationPatient | null>(null);
  const [verificationEnCours, setVerificationEnCours] = useState(false);

  // ✅ CORRECTION : Suppression de la vérification de permission bloquante 
  // pour permettre l'affichage de la page comme convenu (priorité à l'affichage).
  // La gestion des droits UI sera corrigée globalement dans une prochaine étape.

  async function handleCreer() {
    setEnvoi(true);
    setErreur("");
    try {
      await creerDossier({
        patient_nom,
        patient_prenom: patient_prenom || undefined,
        patient_digiid,
        motif,
        diagnostic: diagnostic || undefined,
        hopital: hopital || undefined,
      });
      setEtape("confirmation");
    } catch (e: any) {
      setErreur(e.message_utilisateur || e.message || "Erreur lors de la création du dossier");
    } finally {
      setEnvoi(false);
    }
  }

  return (
    <div className="space-y-8 apparition max-w-4xl mx-auto">
      <nav className="flex items-center gap-2 text-sm text-ardoise-clair">
        <Link href="/medecin/dashboard" className="hover:text-ocre transition-colors">Tableau de bord</Link>
        <span>/</span>
        <span className="text-ardoise font-semibold">Nouveau dossier</span>
      </nav>

      <div>
        <p className="text-ocre font-semibold text-sm uppercase tracking-wider">Espace médical</p>
        <h1 className="mt-1">Nouveau dossier médical</h1>
        <p className="text-ardoise-clair mt-2">Créez un dossier médical pour un patient identifié par son DigiID.</p>
      </div>

      {etape === "recherche" && (
        <Carte titre="Identification du patient">
          <p className="text-sm text-ardoise-clair mb-4">Saisissez le DigiID du citoyen pour vérifier son identité dans le système.</p>
          <div className="max-w-md space-y-4">
            <ChampSaisie 
              libelle="DigiID du patient" 
              value={patient_digiid}
              onChange={(e) => setPatientDigiid(e.target.value)}
              placeholder="Ex: DIG-A1B2C3D4E5F6" 
            />
            {erreur && (
              <div className="bg-terre/10 border-l-4 border-terre p-3 rounded">
                <p className="text-sm text-terre font-medium">{erreur}</p>
              </div>
            )}
            <Bouton 
              variante="primaire" 
              chargement={verificationEnCours}
              disabled={patient_digiid.length < 4 || verificationEnCours}
              onClick={async () => {
                setVerificationEnCours(true);
                setErreur("");
                try {
                  const resultat = await verifierPatient(patient_digiid);
                  if (resultat.trouvé) {
                    setPatientVerifie(resultat);
                    setPatientNom(resultat.nom || "");
                    setPatientPrenom(resultat.prenom || "");
                    setEtape("formulaire");
                  } else {
                    setErreur(`Aucun citoyen trouvé avec le DigiID "${patient_digiid}".`);
                  }
                } catch (e: any) {
                  setErreur(e.message_utilisateur || "Erreur lors de la vérification du DigiID");
                } finally {
                  setVerificationEnCours(false);
                }
              }}
            >
              {verificationEnCours ? "Vérification..." : "Vérifier le DigiID →"}
            </Bouton>
          </div>
        </Carte>
      )}

      {etape === "formulaire" && (
        <>
          <Carte titre="Patient vérifié">
            <div className="grid sm:grid-cols-2 gap-4">
              <div>
                <p className="text-xs uppercase text-ardoise-clair font-semibold">DigiID</p>
                <p className="text-sm font-medium font-mono">{patient_digiid}</p>
              </div>
              <div>
                <p className="text-xs uppercase text-ardoise-clair font-semibold">Nom complet</p>
                <p className="text-sm font-medium">{patient_prenom} {patient_nom}</p>
              </div>
              {patientVerifie?.email && (
                <div>
                  <p className="text-xs uppercase text-ardoise-clair font-semibold">Email</p>
                  <p className="text-sm font-medium">{patientVerifie.email}</p>
                </div>
              )}
            </div>
            <div className="mt-4">
              <span className="inline-flex items-center gap-1 px-2 py-1 bg-succes/10 text-succes text-xs rounded-full font-medium border border-succes/20">
                ✓ Identité citoyenne vérifiée
              </span>
            </div>
          </Carte>

          <Carte titre="Détails de la consultation">
            <div className="space-y-4 max-w-2xl">
              <ChampSaisie 
                libelle="Hôpital / Clinique (optionnel)" 
                value={hopital}
                onChange={(e) => setHopital(e.target.value)}
                placeholder="Ex: Hôpital Principal de Dakar" 
              />
              <ChampSaisie 
                libelle="Motif de la consultation *" 
                value={motif}
                onChange={(e) => setMotif(e.target.value)}
                placeholder="Ex: Consultation de routine, Fièvre, etc." 
              />
              <div>
                <label className="block text-xs uppercase text-ardoise-clair font-semibold mb-1">
                  Diagnostic / Observations
                </label>
                <textarea 
                  value={diagnostic} 
                  onChange={(e) => setDiagnostic(e.target.value)}
                  rows={5} 
                  className="w-full px-3 py-2 border border-ardoise-clair/20 rounded-lg text-sm resize-none focus:outline-none focus:ring-2 focus:ring-lagune/30"
                  placeholder="Description détaillée du diagnostic et des observations..." 
                />
              </div>
              {erreur && (
                <div className="bg-terre/10 border-l-4 border-terre p-3 rounded">
                  <p className="text-sm text-terre font-medium">{erreur}</p>
                </div>
              )}
            </div>
          </Carte>

          <div className="flex gap-3">
            <Bouton variante="primaire" disabled={!motif || envoi} chargement={envoi} onClick={handleCreer}>
              {envoi ? "Création en cours..." : "Créer le dossier"}
            </Bouton>
            <Bouton variante="ghost" onClick={() => {
              setEtape("recherche"); 
              setMotif(""); 
              setDiagnostic(""); 
              setHopital(""); 
              setPatientNom(""); 
              setPatientPrenom(""); 
              setPatientDigiid("");
              setPatientVerifie(null);
            }}>
              Annuler
            </Bouton>
          </div>
        </>
      )}

      {etape === "confirmation" && (
        <Carte titre="Dossier créé avec succès !">
          <p className="text-sm text-ardoise-clair mb-4">
            Le dossier médical de <strong>{patient_prenom} {patient_nom}</strong> ({patient_digiid}) a été enregistré.
          </p>
          <div className="bg-sable p-4 rounded-lg space-y-2 mb-6 border border-ardoise-clair/10">
            <p className="text-sm"><strong>Motif :</strong> {motif}</p>
            {hopital && <p className="text-sm"><strong>Lieu :</strong> {hopital}</p>}
            {diagnostic && <p className="text-sm"><strong>Diagnostic :</strong> {diagnostic}</p>}
          </div>
          <div className="flex gap-3">
            <Link href="/medecin/dossiers">
              <Bouton variante="primaire">Voir tous les dossiers</Bouton>
            </Link>
            <Bouton variante="ghost" onClick={() => {
              setEtape("recherche"); 
              setMotif(""); 
              setDiagnostic(""); 
              setHopital(""); 
              setPatientNom(""); 
              setPatientPrenom(""); 
              setPatientDigiid("");
              setPatientVerifie(null);
            }}>
              Créer un autre dossier
            </Bouton>
          </div>
        </Carte>
      )}

      <Link href="/medecin/dashboard">
        <Bouton variante="ghost">← Retour au tableau de bord</Bouton>
      </Link>
    </div>
  );
}