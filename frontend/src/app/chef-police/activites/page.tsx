...
export default function ChefPoliceActivitesPage() {
  return (
...
function Contenu() {
  ...
  async function chargerActivites() {
    ...
    setErreur("");
    try {
      const data = await listerAgentsPolice({ par_page: 100 });
  ...
  return (
    <div className="min-h-screen space-y-6 apparition pb-20">
      <div>
        ...
        <h1>Activités de l'équipe</h1>
        <p className="text-ardoise-clair mt-2">Suivez l'activité et les performances de vos agents</p>
      </div>
        ...
        <div className="space-y-4">
              ...
              {agents.map((agent) => (
                <div key={agent.id} className="flex items-center justify-between p-4 bg-sable rounded-lg">
                  <div className="flex items-center gap-4">
                    <div className="w-12 h-12 rounded-full bg-terre/10 flex items-center justify-center text-terre font-bold">
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        ...
                        <span className="text-xs text-ardoise-clair">
                          Créé le {new Date(agent.date_creation).toLocaleDateString("fr-FR")}
                        </span>
                      </div>
                    </div>
                  </div>
          ...
          <div className="grid md:grid-cols-3 gap-4">
            <Carte className="text-center p-6">
              <p className="text-3xl font-bold text-lagune">{agents.filter(a => a.est_actif).length}</p>
              <p className="text-xs text-ardoise-clair mt-2">Agents actifs aujourd'hui</p>
            </Carte>
            <Carte className="text-center p-6">
              <p className="text-3xl font-bold text-succes">100%</p>
              <p className="text-xs text-ardoise-clair mt-2">Taux d'activité</p>
            </Carte>
            <Carte className="text-center p-6">
              <p className="text-3xl font-bold text-ocre">{agents.length}</p>
              <p className="text-xs text-ardoise-clair mt-2">Total agents</p>
            </Carte>
          </div>
        </div>
      )}

      <Link href="/chef-police">
