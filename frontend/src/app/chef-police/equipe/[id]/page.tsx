...
export default function AgentPoliceDetailPage() {
  return (
  ...
  if (chargement) {
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <div className="animate-spin w-8 h-8 border-4 border-terre border-t-transparent rounded-full"></div>
      </div>
    );
  }
  ...
  if (erreur || !agent) {
    return (
      <div className="space-y-6">
        <Carte>
          <p className="text-center text-terre">{erreur || "Agent introuvable"}</p>
        </Carte>
        <Link href="/chef-police/equipe">
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6 apparition pb-20">
      <nav className="text-sm text-ardoise-clair flex gap-2">
        <Link href="/chef-police" className="hover:text-terre">Dashboard</Link>
        <span>/</span>
        <Link href="/chef-police/equipe" className="hover:text-terre">Mon équipe</Link>
        <span>/</span>
        <span className="text-ardoise font-semibold">Profil</span>
      </nav>

      <div>
        <p className="text-terre font-semibold text-sm uppercase tracking-wider">Profil agent</p>
        <h1>{agent.prenom} {agent.nom}</h1>
      </div>

      <Carte>
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-6">
          <div className="w-20 h-20 rounded-full bg-terre/10 flex items-center justify-center text-terre text-3xl font-bold flex-shrink-0">
          ...
          <div className="flex-1 space-y-2">
            ...
            <div className="flex flex-wrap gap-2">
              <Badge variante="lagune" taille="petit">Agent Police</Badge>
