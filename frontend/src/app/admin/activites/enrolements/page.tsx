  ...
  return (
    <div className="space-y-8 apparition">
      <div>
        ...
        <p className="text-ardoise-clair mt-2">Tous les enrolements effectues par les agents terrain.</p>
      </div>

      <NavigationActivites active="enrolements" />

      {/* Stats */}
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <div className="carte border-l-4 border-lagune/30">
          <p className="text-3xl font-bold text-lagune">{stats.total}</p>
          <p className="text-xs uppercase text-ardoise-clair font-semibold">Total</p>
        </div>
        <div className="carte border-l-4 border-ocre/30">
          <p className="text-3xl font-bold text-ocre">{stats.en_attente}</p>
          <p className="text-xs uppercase text-ardoise-clair font-semibold">En attente</p>
        </div>
        <div className="carte border-l-4 border-green-400/30">
          <p className="text-3xl font-bold text-green-600">{stats.valide}</p>
          <p className="text-xs uppercase text-ardoise-clair font-semibold">Valides</p>
        </div>
        <div className="carte border-l-4 border-terre/30">
          <p className="text-3xl font-bold text-terre">{stats.rejete}</p>
          <p className="text-xs uppercase text-ardoise-clair font-semibold">Rejetes</p>
        </div>
      </div>
            ...
            {filtres.map((f) => (
              <button
                ...
                className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all ${
                  filtreStatut === f ? "bg-lagune text-white" : "bg-sable text-ardoise-clair hover:bg-sable/80"
                }`}
              >
