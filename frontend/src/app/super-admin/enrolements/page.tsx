...
/**
 * Page super admin des enrôlements — même contenu que l'admin + export CSV.
 */
...
function Contenu() {
  ...
  function exporterCSV() {
    const entetes = ["Prenom,Nom,Telephone,Email,DigiID,Agent,Statut,Date,Valide le,Notes,Scan CNI,Biometrie"];
    const lignes = enrolementsFiltres.map(
      (e) =>
        `${e.citoyen_prenom},${e.citoyen_nom},${e.citoyen_telephone || ""},${e.citoyen_email || ""},${e.citoyen_digiid || ""},${e.agent_nom},${e.statut},${new Date(e.date_enrolement).toLocaleDateString("fr-FR")},${e.date_validation ? new Date(e.date_validation).toLocaleDateString("fr-FR") : ""},${e.notes || ""},${e.scan_cni ? "Oui" : "Non"},${e.capture_biometrique ? "Oui" : "Non"}`
    );
