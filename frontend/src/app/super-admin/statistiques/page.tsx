...
function Contenu() {
    ...
    return (
      <div className="space-y-4">
        <header>
          ...
          <h1 className="mt-1 text-2xl">Statistiques détaillées</h1>
        </header>
        <p className="text-ardoise-clair italic text-center py-6">Chargement des statistiques...</p>
      </div>
    );
  }
  ...
  const gererExportCSV = () => {
    ...
    lignes.push(["2FA activée", String(utilisateurs.total_2fa_actif)]);
    lignes.push(["Taux 2FA", Math.round(utilisateurs.taux_activation_2fa) + "%"]);
    lignes.push(["Emails vérifiés", String(utilisateurs.total_verifies_email)]);
    ...
    lignes.push(["Événements audit", String(stats.total_evenements_audit)]);
  ...
  return (
    <div className="space-y-4">
      ...
      <section className="space-y-2">
        <h2 className="text-base font-bold text-ardoise">Utilisateurs</h2>
        ...
        <div className="grid md:grid-cols-2 gap-3">
          <Carte titre="Vue d'ensemble">
            ...
            <StatLigne libelle="Supprimés" valeur={utilisateurs.total_supprimes} couleur="neutre" />
          </Carte>
          <Carte titre="Répartition par rôle">
      ...
      {/* Sécurité */}
      <section className="space-y-2">
        <h2 className="text-base font-bold text-ardoise">Sécurité</h2>

        <div className="grid md:grid-cols-2 gap-3">
          <Carte titre="Double authentification (2FA)">
            <StatLigne libelle="2FA activée" valeur={utilisateurs.total_2fa_actif} couleur="ocre" />
            <StatLigne libelle="2FA désactivée" valeur={utilisateurs.total_2fa_inactif} couleur="terre" />
            <StatLigne libelle="Taux 2FA" valeur={Math.round(utilisateurs.taux_activation_2fa) + "%"} couleur="ocre" />
          </Carte>
          <Carte titre="Vérification email">
            <StatLigne libelle="Emails vérifiés" valeur={utilisateurs.total_verifies_email} couleur="lagune" />
            <StatLigne libelle="Non vérifiés" valeur={utilisateurs.total_non_verifies_email} couleur="neutre" />
            <StatLigne libelle="Taux vérification" valeur={Math.round(utilisateurs.taux_verification_email) + "%"} couleur="lagune" />
          </Carte>
        </div>
      </section>
      ...
      {/* Audit */}
      <section className="space-y-2">
        <h2 className="text-base font-bold text-ardoise">Activité d'audit</h2>

        <Carte>
          <div className="grid sm:grid-cols-2 gap-3">
            <StatLigne libelle="Total événements" valeur={stats.total_evenements_audit} couleur="terre" />
            <StatLigne libelle="Événements aujourd'hui" valeur={stats.evenements_aujourd_hui} couleur="lagune" />
          </div>
        </Carte>
      </section>

      <section className="space-y-2">
        <h2 className="text-base font-bold text-ardoise">Sessions</h2>
        <Carte>
          <div className="grid sm:grid-cols-2 gap-3">
            <StatLigne libelle="Actives" valeur={sessions.sessions_actives} couleur="succes" />
            <StatLigne libelle="Expirées" valeur={sessions.sessions_expirees} couleur="neutre" />
            <StatLigne libelle="Révoquées" valeur={sessions.sessions_revoquees} couleur="terre" />
      ...
      <section className="space-y-2">
        <h2 className="text-base font-bold text-ardoise">Scores de confiance</h2>
        <Carte>
      ...
      {/* Recommandations */}
      <section className="space-y-2">
        <h2 className="text-base font-bold text-ardoise">Recommandations</h2>

        <Carte>
          <ul className="space-y-2">
            {utilisateurs.taux_activation_2fa < 80 && (
              <li className="flex gap-2 p-2.5 bg-ocre/10 rounded-lg border border-ocre/20">
                <div className="text-sm">
                  <p className="font-semibold text-ocre text-sm">Augmenter l'activation 2FA</p>
                  <p className="text-ardoise-clair text-xs mt-0.5">Seulement {Math.round(utilisateurs.taux_activation_2fa)}% des utilisateurs ont 2FA active.</p>
                </div>
              </li>
            )}
            {utilisateurs.taux_verification_email < 70 && (
              <li className="flex gap-2 p-2.5 bg-lagune/10 rounded-lg border border-lagune/20">
                <div className="text-sm">
                  <p className="font-semibold text-lagune text-sm">Améliorer la vérification email</p>
                  <p className="text-ardoise-clair text-xs mt-0.5">{100 - Math.round(utilisateurs.taux_verification_email)}% des utilisateurs n'ont pas vérifié leur email.</p>
                </div>
              </li>
            )}
            {tauxActivite < 60 && (
              <li className="flex gap-2 p-2.5 bg-terre/10 rounded-lg border border-terre/20">
                <div className="text-sm">
                  <p className="font-semibold text-terre text-sm">Faible activité utilisateurs</p>
                  <p className="text-ardoise-clair text-xs mt-0.5">{inactifs} utilisateurs sont inactifs.</p>
                </div>
              </li>
            )}
            {utilisateurs.taux_activation_2fa >= 80 && utilisateurs.taux_verification_email >= 70 && tauxActivite >= 60 && (
              <li className="flex gap-2 p-2.5 bg-green-100 rounded-lg border border-green-300">
                <div className="text-sm">
                  <p className="font-semibold text-green-900 text-sm">Excellent état du système</p>
                  <p className="text-green-800 text-xs mt-0.5">Tous les indicateurs sont au vert.</p>
                </div>
              </li>
            )}
          </ul>
...
function StatLigne({
  ...
  return (
    <div className="p-3 bg-sable rounded-lg">
      <p className="text-[10px] uppercase text-ardoise-clair font-semibold mb-1">
        {libelle}
      </p>
      <div className="flex justify-between items-baseline">
        <p className={`text-2xl font-bold ${couleurClasses[couleur]}`}>
          {valeur}
        </p>
        {pourcentage !== undefined && (
          <p className={`text-base font-semibold ${couleurClasses[couleur]}`}>
            {pourcentage}%
          </p>
        )}
      </div>
