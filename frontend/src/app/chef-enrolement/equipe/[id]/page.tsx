...
export default function AgentEnrolementDetailPage() {
  return (
...
function Contenu() {
  ...
  async function chargerAgent() {
    ...
    setErreur("");
    try {
      const data = await listerAgentsEnrolement({ par_page: 1000 });
      const agentTrouve = data.agents?.find((a) => a.id === agentId);
      if (agentTrouve) {
        setAgent(agentTrouve);
      } else {
        setErreur("Agent terrain introuvable.");
      }
    } catch (error: any) {
  ...
  if (erreur || !agent) {
    return (
      <div className="space-y-6">
        <Carte>
          <p className="text-center text-terre">{erreur || "Agent terrain introuvable"}</p>
        </Carte>
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6 apparition pb-20">
      <nav className="text-sm text-ardoise-clair flex gap-2">
        <Link href="/chef-enrolement" className="hover:text-lagune">Dashboard</Link>
        <span>/</span>
        <Link href="/chef-enrolement/equipe" className="hover:text-lagune">Agents terrain</Link>
        <span>/</span>
        <span className="text-ardoise font-semibold">Profil</span>
      </nav>

      <div>
        <p className="text-lagune font-semibold text-sm uppercase tracking-wider">Profil agent terrain</p>
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-6">
          ...
          <div className="flex-1 space-y-2">
            ...
            <div className="flex flex-wrap gap-2">
              <Badge variante="lagune" taille="petit">Agent Terrain</Badge>
