# -*- coding: utf-8 -*-
"""Service métier pour le module Police — version complète."""
import time
...
async def creer_verification(
    ...
    """Crée une nouvelle vérification d'identité avec motif."""
...
async def obtenir_verifications(
    session: AsyncSession,
    officier_id: Optional[UUID] = None,
    limite: int = 50,
    page: int = 1,
    ...
    if officier_id:
        query = query.where(VerificationPolice.officier_id == officier_id)
...
async def rechercher_personne(
    ...
    for utilisateur in utilisateurs:
        digiid = utilisateur.digiid_public or ""
        prenom = dechiffrer_donnee(utilisateur.prenom_chiffre) if utilisateur.prenom_chiffre else ""
        nom = dechiffrer_donnee(utilisateur.nom_chiffre) if utilisateur.nom_chiffre else ""
        email = dechiffrer_donnee(utilisateur.email_chiffre) if utilisateur.email_chiffre else ""
        ...
        if type_recherche in ("tout", "cni"):
            result_cni = await session.execute(
                ...
                .where(VerificationCNI.utilisateur_id == utilisateur.id, VerificationCNI.est_supprime == False, VerificationCNI.numero_cni.ilike(f"%{query_clean}%"))
                .limit(1)
            )
        ...
        if not correspond:
            continue
        if filtre_score_min is not None and (utilisateur.score_actuel or 0) < filtre_score_min:
            continue
        if filtre_score_max is not None and (utilisateur.score_actuel or 0) > filtre_score_max:
            continue
        if filtre_ville and filtre_ville.lower() not in (utilisateur.ville or "").lower():
            continue
        if utilisateur.id in utilisateurs_vus:
            continue
        utilisateurs_vus.add(utilisateur.id)
        docs = await session.execute(
            select(DocumentIdentite).where(DocumentIdentite.utilisateur_id == utilisateur.id, DocumentIdentite.est_actif == True)
        )
        ...
        result_cni = await session.execute(
            select(VerificationCNI)
            .where(VerificationCNI.utilisateur_id == utilisateur.id, VerificationCNI.est_supprime == False)
        ...
        resultats.append({
            ...
            "email": email,
            "telephone": telephone,
            "score": utilisateur.score_actuel or 0,
            "est_actif": utilisateur.est_actif,
            "est_verifie": utilisateur.est_cni_verifiee or utilisateur.est_visage_verifie,
            "ville": utilisateur.ville or "",
            "pays": utilisateur.pays or "",
            "photo_url": None,
    ...
    if officier_id:
        await _enregistrer_recherche(session, officier_id, type_recherche, query_clean, total)
...
async def obtenir_profil_personne(
    ...
    return {
        ...
        "date_inscription": utilisateur.cree_le,
        "documents": documents,
...
async def creer_signalement(session: AsyncSession, officier_id: UUID, data: dict) -> SignalementFraude:
    """Crée un nouveau signalement de fraude."""
    data.pop("personne_id", None)
...
async def obtenir_signalements(
    ...
    """Liste les signalements avec pagination et filtre."""
    query = select(SignalementFraude).order_by(SignalementFraude.date_signalement.desc())
    if officier_id:
        query = query.where(SignalementFraude.officier_id == officier_id)
    if statut:
...
async def traiter_signalement(
    ...
    signalement.traite_par_id = officier_id
...
async def creer_note(session: AsyncSession, officier_id: UUID, data: dict) -> NoteInternePolice:
    """Crée une note interne sur une personne."""
    note = NoteInternePolice(officier_id=officier_id, **data)
    session.add(note)
    await session.commit()
...
async def obtenir_notes(
    ...
    """Liste les notes internes avec filtres."""
    query = select(NoteInternePolice).order_by(NoteInternePolice.date_creation.desc())
    if officier_id:
        query = query.where(NoteInternePolice.officier_id == officier_id)
    if personne_digiid:
...
async def modifier_note(session: AsyncSession, note_id: UUID, officier_id: UUID, data: dict) -> NoteInternePolice:
    """Modifie une note interne."""
    result = await session.execute(select(NoteInternePolice).where(NoteInternePolice.id == note_id, NoteInternePolice.officier_id == officier_id))
...
async def supprimer_note(session: AsyncSession, note_id: UUID, officier_id: UUID) -> bool:
...
async def creer_alerte(session: AsyncSession, officier_id: UUID, data: dict) -> AlertePolice:
    """Crée une alerte pour un officier."""
    alerte = AlertePolice(officier_id=officier_id, **data)
    session.add(alerte)
    await session.commit()
...
async def obtenir_alertes(
    ...
    """Liste les alertes d'un officier."""
    query = select(AlertePolice).where(AlertePolice.officier_id == officier_id, AlertePolice.est_active == True).order_by(AlertePolice.date_creation.desc())
    if non_lues_seulement:
        query = query.where(AlertePolice.est_lue == False)
    count_query = select(func.count()).select_from(query.subquery())
    total_result = await session.execute(count_query)
    total = total_result.scalar() or 0
    non_lues_query = select(func.count()).where(AlertePolice.officier_id == officier_id, AlertePolice.est_lue == False, AlertePolice.est_active == True)
...
async def marquer_alerte_lue(session: AsyncSession, alerte_id: UUID, officier_id: UUID) -> AlertePolice:
    """Marque une alerte comme lue."""
    result = await session.execute(select(AlertePolice).where(AlertePolice.id == alerte_id, AlertePolice.officier_id == officier_id))
...
async def creer_alerte_pour_tous(
    ...
    for off in officiers:
        alerte = AlertePolice(officier_id=off.id, type_alerte=type_alerte, titre=titre, message=message, niveau=niveau, donnees_liees=donnees_liees)
        session.add(alerte)
        nb_alertes += 1
    await session.commit()
...
async def obtenir_statistiques(session: AsyncSession, officier_id: UUID) -> dict:
    """Obtient les statistiques pour le dashboard."""
    aujourdhui = datetime.now(UTC).replace(hour=0, minute=0, second=0, microsecond=0)
    result = await session.execute(select(func.count()).where(VerificationPolice.officier_id == officier_id))
    total_verifs = result.scalar() or 0
    result = await session.execute(select(func.count()).where(VerificationPolice.officier_id == officier_id, VerificationPolice.date_verification >= aujourdhui))
    verifs_aujourdhui = result.scalar() or 0
    result = await session.execute(select(func.count()).where(SignalementFraude.officier_id == officier_id))
    total_signalements = result.scalar() or 0
    result = await session.execute(select(func.count()).where(SignalementFraude.officier_id == officier_id, SignalementFraude.statut == "en_cours"))
    signalements_en_cours = result.scalar() or 0
    result = await session.execute(select(func.count()).where(SignalementFraude.officier_id == officier_id, SignalementFraude.statut == "traite"))
    signalements_traites = result.scalar() or 0
    result = await session.execute(select(func.count()).where(AlertePolice.officier_id == officier_id, AlertePolice.est_lue == False, AlertePolice.est_active == True))
    alertes_non_lues = result.scalar() or 0
    result = await session.execute(select(func.count()).where(NoteInternePolice.officier_id == officier_id))
    notes_total = result.scalar() or 0
    result = await session.execute(select(func.count()).where(HistoriqueRecherchePolice.officier_id == officier_id))
    personnes_recherchees = result.scalar() or 0
    result = await session.execute(select(func.count()).where(VerificationPolice.officier_id == officier_id, VerificationPolice.resultat == "confirme"))
    confirme = result.scalar() or 0
    taux_confirmation = round((confirme / total_verifs) * 100, 1) if total_verifs > 0 else None
    verifs, _ = await obtenir_verifications(session, officier_id, limite=5)
    signalements, _ = await obtenir_signalements(session, officier_id, limite=5)
    alertes, _, _ = await obtenir_alertes(session, officier_id, limite=5)
    return {
...
async def obtenir_points_carte(session: AsyncSession, officier_id: UUID, limite: int = 100) -> dict:
    """Obtient les points pour la carte géographique des vérifications."""
    result = await session.execute(
        select(VerificationPolice)
        .where(VerificationPolice.officier_id == officier_id, VerificationPolice.localisation_lat.isnot(None), VerificationPolice.localisation_lng.isnot(None))
    ...
    for v in verifications:
        points.append({"lat": v.localisation_lat, "lng": v.localisation_lng, "adresse": v.localisation_adresse, "titre": f"{v.personne_nom or v.personne_digiid} - {v.type_verification}", "type": v.resultat or "en_cours", "date": v.date_verification, "verification_id": v.id})
    ...
    return {"points": points, "total": len(points), "centre_lat": centre_lat, "centre_lng": centre_lng}
...
async def scanner_qr(session: AsyncSession, digiid: str, officier_id: UUID) -> dict:
    """Traite un scan de QR code citoyen."""
    result = await session.execute(select(Utilisateur).where(Utilisateur.digiid_public == digiid))
    utilisateur = result.scalar_one_or_none()
    if not utilisateur:
        raise ErreurRessourceIntrouvable(f"DigiID {digiid} introuvable.", message_utilisateur="Ce code QR n'est pas reconnu dans le système.")
    prenom = dechiffrer_donnee(utilisateur.prenom_chiffre) if utilisateur.prenom_chiffre else ""
    nom = dechiffrer_donnee(utilisateur.nom_chiffre) if utilisateur.nom_chiffre else ""
    docs = await session.execute(select(DocumentIdentite).where(DocumentIdentite.utilisateur_id == utilisateur.id, DocumentIdentite.est_actif == True))
    documents = []
    for d in docs.scalars().all():
        documents.append({"type_document": d.type_document, "numero": d.numero_document, "nom_complet": d.nom_complet, "date_expiration": d.date_expiration.isoformat() if d.date_expiration else None, "est_valide": d.date_expiration is None or (d.date_expiration if isinstance(d.date_expiration, datetime) else datetime.combine(d.date_expiration, datetime.min.time())) >= datetime.now(UTC)})
    return {"digiid": utilisateur.digiid_public or "", "nom": f"{prenom} {nom}".strip(), "email": dechiffrer_donnee(utilisateur.email_chiffre) if utilisateur.email_chiffre else "", "photo_url": None, "est_actif": utilisateur.est_actif, "est_verifie": utilisateur.est_cni_verifiee or utilisateur.est_visage_verifie, "documents": documents}
...
async def obtenir_historique_complet(session: AsyncSession, officier_id: UUID, type_historique: Optional[str] = None, limite: int = 50) -> dict:
    """Obtient l'historique complet des activités de l'officier."""
    resultats = {}
    if not type_historique or type_historique == "verifications":
        verifs, total_v = await obtenir_verifications(session, officier_id, limite)
        resultats["verifications"] = [{"id": v.id, "personne_digiid": v.personne_digiid, "personne_nom": v.personne_nom, "type_verification": v.type_verification, "motif_verification": v.motif_verification, "resultat": v.resultat, "notes": v.notes, "date_verification": v.date_verification, "est_signalement_fraude": v.est_signalement_fraude} for v in verifs]
        resultats["total_verifications"] = total_v
    if not type_historique or type_historique == "signalements":
        signalements, total_s = await obtenir_signalements(session, officier_id, limite=limite)
        resultats["signalements"] = [{"id": s.id, "personne_digiid": s.personne_digiid, "motif": s.motif, "statut": s.statut, "priorite": s.priorite, "date_signalement": s.date_signalement} for s in signalements]
        resultats["total_signalements"] = total_s
    if not type_historique or type_historique == "recherches":
        result = await session.execute(select(HistoriqueRecherchePolice).where(HistoriqueRecherchePolice.officier_id == officier_id).order_by(HistoriqueRecherchePolice.date_recherche.desc()).limit(limite))
        recherches = result.scalars().all()
        resultats["recherches"] = [{"id": r.id, "type_recherche": r.type_recherche, "terme_recherche": r.terme_recherche, "resultats_trouves": r.resultats_trouves, "date_recherche": r.date_recherche} for r in recherches]
...
async def generer_rapport(
    ...
    rapport = {"officier_id": str(officier_id), "date_generation": datetime.now(UTC).isoformat(), "periode": {"debut": date_debut.isoformat() if date_debut else None, "fin": date_fin.isoformat() if date_fin else None}, "donnees": {}}
    base_query_verif = select(VerificationPolice).where(VerificationPolice.officier_id == officier_id)
    base_query_signal = select(SignalementFraude).where(SignalementFraude.officier_id == officier_id)
    if date_debut:
    ...
    if "verifications" in type_donnees:
        ...
        rapport["donnees"]["verifications"] = [{"personne_digiid": v.personne_digiid, "personne_nom": v.personne_nom, "type": v.type_verification, "motif": v.motif_verification, "resultat": v.resultat, "date": v.date_verification.isoformat()} for v in verifs]
    ...
    if "signalements" in type_donnees:
        ...
        rapport["donnees"]["signalements"] = [{"personne_digiid": s.personne_digiid, "motif": s.motif, "statut": s.statut, "priorite": s.priorite, "date": s.date_signalement.isoformat()} for s in signalements]
...
async def comparer_photos(
    ...
    Utilise l'embedding facial basé sur SHA-512.
    """
    import base64
    ...
    try:
        ...
        temps = (time.time() - debut) * 1000
        return {"score_similarite": round(float(score), 4), "est_compatible": bool(score >= 0.75), "seuil_requis": 0.75, "temps_analyse_ms": round(temps, 2), "details": {"algorithme": "sha512_embedding", "dimension": 512}}
    except Exception as e:
        score = 0.5
        return {"score_similarite": score, "est_compatible": score >= 0.75, "seuil_requis": 0.75, "temps_analyse_ms": 0, "details": {"mode": "fallback", "erreur": str(e)[:200], "message": "Algorithme principal indisponible, fallback utilisé"}}
