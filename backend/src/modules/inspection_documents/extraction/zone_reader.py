    ...
    try:
        ...
        try:
            reponse = await asyncio.wait_for(
                appeler_llm_vision(image_base64=img_b64, prompt=prompt, mime_type="image/jpeg"),
                timeout=10.0
            )
...
async def lire_zones_non_structurees(bandes: Dict[str, bytes]) -> Dict[str, Optional[str]]:
    resultats = {}
    prompt_nom = "Tu es un OCR pur. Lis UNIQUEMENT le nom de famille (en majuscules) sur cette petite image. Réponds STRICTEMENT avec le mot du nom, ou 'null' si illisible."
    prompt_prenom = "Tu es un OCR pur. Lis le prénom sur cette image. Réponds STRICTEMENT avec le mot du prénom, ou 'null' si illisible."
    prompt_lieu = "Tu es un OCR pur. Lis le lieu de naissance ou la ville sur cette image. Réponds STRICTEMENT avec le mot du lieu, ou 'null' si illisible."
