# -*- coding: utf-8 -*-
"""Adaptateur Carte de séjour : enveloppe ocr_carte_sejour dans la réponse unifiée."""
...
from src.modules.ocr_carte_sejour import service as carte_sejour_service
from src.noyau.exceptions import ErreurDigiID


async def adapter_carte_sejour(
    ...
    *,
    ...
    """Extrait une carte / titre de séjour et renvoie la réponse unifiée."""
    try:
        reponse = await carte_sejour_service.traiter_upload_carte_sejour(
            session=session, utilisateur=utilisateur, fichier=fichier,
        )
    except ErreurDigiID as e:
        return reponse_unifiee(
            "carte_sejour", statut="rejete",
