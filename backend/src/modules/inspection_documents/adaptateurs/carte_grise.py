# -*- coding: utf-8 -*-
"""Adaptateur Carte grise : enveloppe ocr_carte_grise dans la réponse unifiée."""
...
from src.modules.ocr_carte_grise import service as carte_grise_service
from src.noyau.exceptions import ErreurDigiID


async def adapter_carte_grise(
    ...
    *,
    ...
    """Extrait une carte grise et renvoie la réponse unifiée."""
    try:
        reponse = await carte_grise_service.traiter_upload_carte_grise(
            session=session, utilisateur=utilisateur, fichier=fichier,
        )
    except ErreurDigiID as e:
        return reponse_unifiee(
            "carte_grise", statut="rejete",
