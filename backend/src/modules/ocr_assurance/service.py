...
async def traiter_upload_assurance(
    ...
    # 1. Analyse OCR (à connecter au moteur commun ocr_communs/engine.py)
    ...
    message = (
        "Assurance scannée avec succès." if succes
        else "L'OCR n'a pas pu extraire suffisamment de données."
    )
    
    # TODO: Enregistrement en base via DocumentIdentite avec type_document="assurance_auto"
...
async def obtenir_historique_assurance(
    ...
    # TODO: Implémenter avec le modèle DocumentIdentite filtré sur type_document="assurance_auto"
    return ListeVerificationsAssurance(historique=[], total=0)