...
class Rapport(Base):
    ...
    mission_id = Column(UUID(as_uuid=True), ForeignKey("missions_terrain.id", ondelete="SET NULL"), nullable=True)
    programme_id = Column(UUID(as_uuid=True), ForeignKey("programmes_ong.id", ondelete="SET NULL"), nullable=True)
    ...
    # Audit
    # ✅ CORRECTION : ForeignKey vers "utilisateur.id" (singulier) et type UUID
    cree_par = Column(UUID(as_uuid=True), ForeignKey("utilisateur.id", ondelete="CASCADE"), nullable=False)
